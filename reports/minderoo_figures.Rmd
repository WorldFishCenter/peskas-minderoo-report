...
period_dates <- lubridate::as_date(c("2020-06-01", "2021-06-18"))
reporting_dates <- lubridate::as_date(c("2017-06-01", "2021-06-18"))
...
```{r, fig.height=4.5, fig.width=8}
landings %>%
  dplyr::filter(!is.na(total_catch_value)) %>%
  ggplot(aes(x = month, y = total_catch_value)) +
...
landings %>%
  ...
  annotate("segment", as.Date("2020-06-01"), xend = as.Date("2021-06-18"), y = -1, yend = -1, colour = "#1a4985",
           size=1, alpha=0.8,arrow=arrow(ends = "both",length = unit(0.075, "inches")))+
  annotate("segment", as.Date("2020-06-01"), xend = as.Date("2021-06-18"), y = 101, yend = 101, colour = "#1a4985",
           size=1, alpha=0.8,arrow=arrow(ends = "both",length = unit(0.075, "inches")))+
  scale_x_date(date_breaks = "3 month", minor_breaks = NULL, labels = label_date, limits = set_date_limits) +
...
p1 <-
  obs_per_sites %>%
  ggplot() +
  geom_point(aes(x = reorder(as.numeric(landing_site_name),tot_obs_site),y=tot_obs_site),
             alpha=0.5,na.rm = TRUE,size=2.5) +
  geom_segment(aes(y = 0,x = landing_site_name,yend = tot_obs_site,xend = landing_site_name),
               color = "black",alpha=0.5)+
  theme_minimal()  +
  labs(x = "", y = "Number of landings",
       caption = "Distribution of species catches and landings among the different landing sites") +
  theme_minimal() +
  coord_flip()+
...
p2 <-
  ...
  dplyr::count() %>% 
  dplyr::left_join(species_freq) %>%
  dplyr::left_join(obs_per_sites) %>%
  dplyr::mutate(n_perc=(n/tot_obs_site)*100,
                species=dplyr::case_when(cumspe>75~"other",
                                         TRUE~species)) %>% 
  dplyr::group_by(landing_site_name,species) %>% 
  dplyr::summarise(n_perc=sum(n_perc),n=sum(n)) %>% 
ggplot(aes(x = reorder(landing_site_name,n),y=n_perc,fill=species)) +
...
plot_grid(p2,
          ...
          rel_widths=c(2.5,1),align="h")

title <- ggdraw() + draw_label("Site 6 receives most of the landings. The species distribution 
...
```{r, fig.height=4.5, fig.width=8}
pds_trips %>%
  dplyr::filter(!is.na(`Duration (Seconds)`)) %>% 
