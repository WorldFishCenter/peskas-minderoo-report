...
landings %>%
  ...
  annotate("segment", as.Date("2020-06-01"), xend = as.Date("2021-06-30"), y = -1, yend = -1, colour = "#1a4985",
           size=1, alpha=0.8,arrow=arrow(ends = "both",length = unit(0.075, "inches")))+
  annotate("segment", as.Date("2020-06-01"), xend = as.Date("2021-06-30"), y = 101, yend = 101, colour = "#1a4985",
           size=1, alpha=0.8,arrow=arrow(ends = "both",length = unit(0.075, "inches")))+
  scale_x_date(date_breaks = "3 month", minor_breaks = NULL, labels = label_date, limits = set_date_limits) +
...
site_col <- ifelse(lev_data1 %in% new_sites, "#1a4985", "grey30")
site_face <- ifelse(lev_data1 %in% new_sites, "bold", "plain")
